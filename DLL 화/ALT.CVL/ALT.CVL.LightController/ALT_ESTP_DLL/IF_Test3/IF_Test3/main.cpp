...
// ESTP Data
unsigned short g_StrobeArray[MAX_CHANNEL_64] ;
unsigned short g_Strobe = 0 ;
char g_PageIndex = 0 ;
	...
	case WM_COMMAND :	
		...
		case IDC_START :
			...
			{				
				...
				g_NetIndex = ALT_ESTP_Create ( ) ;
				if ( g_NetIndex == -1 )
					return FALSE ;

				//if ( ! ALT_ESTP_Lan_Start ( g_NetIndex, TRUE, "192.168.10.10", 1000, MAX_CHANNEL_4 ) )
				//	MessageBox ( NULL, TEXT ( "ALT_LSPE_Lan_Start 1 Error!" ), TEXT ( "ALT-ESTP" ), MB_OK ) ;
				if ( ! ALT_ESTP_Uart_Start ( g_NetIndex, "\\\\.\\COM3", 115200, 4 ) )
...
void DataViewProc ( void *Data )
{
	...
	while ( 1 )
	{
		...
		DataSize = 4400 ;
		if ( ALT_ESTP_Protocol_LoadW ( g_NetIndex, &Time, Target, &DataSize, &SendMode ) )
		{
...
void CommProc ( void *Data )
{
	...
	while ( 1 )
	{
		...
		if ( ! ALT_ESTP_IsTcpConnect ( g_NetIndex ) && 
			! ALT_ESTP_IsUdpConnect ( g_NetIndex ) &&
			! ALT_ESTP_IsUartConnect ( g_NetIndex ) )
			continue ;		
		
		if ( ! ALT_ESTP_ReadIPAddress ( g_NetIndex, g_IP, g_NetMask, g_GateWay, &g_PortNum ) )
			MessageBox ( NULL, TEXT ( "ALT_ESTP_ReadIPAddress Error!" ), TEXT ( "ALT-ESTP" ), MB_OK ) ;
		if ( ! ALT_ESTP_ReadMACAddress ( g_NetIndex, g_MAC ) )
			MessageBox ( NULL, TEXT ( "ALT_ESTP_ReadMACAddress Error!" ), TEXT ( "ALT-ESTP" ), MB_OK ) ;

		if ( ! ALT_ESTP_StrobeValueSend ( g_NetIndex, g_PageIndex, g_ChannelIndex, g_Strobe ) )
			MessageBox ( NULL, TEXT ( "ALT_ESTP_StrobeValueSend Error!" ), TEXT ( "ALT-ESTP" ), MB_OK ) ;
		if ( ! ALT_ESTP_AllStrobeValueSend ( g_NetIndex, g_PageIndex, g_StrobeArray ) )
			MessageBox ( NULL, TEXT ( "ALT_ESTP_AllStrobeValueSend Error!" ), TEXT ( "ALT-ESTP" ), MB_OK ) ;
		if ( ! ALT_ESTP_AllStrobeValueRead ( g_NetIndex, g_PageIndex, g_StrobeArray ) )
			MessageBox ( NULL, TEXT ( "ALT_ESTP_AllStrobeValueRead Error!" ), TEXT ( "ALT-ESTP" ), MB_OK ) ;

		if ( ! ALT_ESTP_PageIndexSend ( g_NetIndex, g_PageIndex ) )
			MessageBox ( NULL, TEXT ( "ALT_ESTP_PageIndexSend Error!" ), TEXT ( "ALT-ESTP" ), MB_OK ) ;
		if ( ! ALT_ESTP_PageIndexRead ( g_NetIndex, &g_PageIndex ) )
			MessageBox ( NULL, TEXT ( "ALT_ESTP_PageIndexRead Error!" ), TEXT ( "ALT-ESTP" ), MB_OK ) ;
		if ( ! ALT_ESTP_ConfigModeRead ( g_NetIndex, &g_ChannelNum, &g_TriggerMode, &g_EncoderMode ) )
			MessageBox ( NULL, TEXT ( "ALT_ESTP_ConfigModeRead Error!" ), TEXT ( "ALT-ESTP" ), MB_OK ) ;		
