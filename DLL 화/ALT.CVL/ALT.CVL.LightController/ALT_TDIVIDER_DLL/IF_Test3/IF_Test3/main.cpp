...
// TDIVIDER Data
unsigned long g_FVersion ;
unsigned long g_LVersion ;
unsigned long g_SEncCount ;
unsigned long g_AEncCount ;
unsigned long g_DEncCount ;
unsigned long g_ZEncCount ;
	...
	case WM_COMMAND :	
		...
		case IDC_START :
			...
			{				
				...
				g_NetIndex = ALT_TDIV_Create ( ) ;
				if ( g_NetIndex == -1 )
					return FALSE ;

				//if ( ! ALT_TDIV_Lan_Start ( g_NetIndex, TRUE, "192.168.10.10", 1000 ) )
				//	MessageBox ( NULL, TEXT ( "ALT_TDIV_Lan_Start 1 Error!" ), TEXT ( "ALT-TDIV" ), MB_OK ) ;
				if ( ! ALT_TDIV_Uart_Start ( g_NetIndex, "\\\\.\\COM3", 115200 ) )
...
void DataViewProc ( void *Data )
{
	...
	while ( 1 )
	{
		...
		DataSize = 4400 ;
		if ( ALT_TDIV_Protocol_LoadW ( g_NetIndex, &Time, Target, &DataSize, &SendMode ) )
		{
...
void CommProc ( void *Data )
{
	...
	while ( 1 )
	{
		...
		if ( ! ALT_TDIV_IsTcpConnect ( g_NetIndex ) && 
			! ALT_TDIV_IsUdpConnect ( g_NetIndex ) &&
			! ALT_TDIV_IsUartConnect ( g_NetIndex ) )
			continue ;		
		
		if ( ! ALT_TDIV_ReadIPAddress ( g_NetIndex, g_IP, g_NetMask, g_GateWay, &g_PortNum ) )
			MessageBox ( NULL, TEXT ( "ALT_TDIV_ReadIPAddress Error!" ), TEXT ( "ALT-TDIV" ), MB_OK ) ;
		if ( ! ALT_TDIV_ReadMACAddress ( g_NetIndex, g_MAC ) )
			MessageBox ( NULL, TEXT ( "ALT_TDIV_ReadMACAddress Error!" ), TEXT ( "ALT-TDIV" ), MB_OK ) ;

		if ( ! ALT_TDIV_VersionInfoRead ( g_NetIndex, &g_FVersion, &g_LVersion ) )
			MessageBox ( NULL, TEXT ( "ALT_TDIV_VersionInfoRead Error!" ), TEXT ( "ALT-TDIV" ), MB_OK ) ;
		if ( ! ALT_TDIV_EncCountRead ( g_NetIndex, &g_SEncCount, &g_AEncCount, &g_DEncCount, &g_ZEncCount ) )
			MessageBox ( NULL, TEXT ( "ALT_TDIV_EncCountRead Error!" ), TEXT ( "ALT-TDIV" ), MB_OK ) ;
